...
void DT_Data_Receive_Prepare(uint8 data)
{
	...
	else if(state==1&&(data==0xAA || data==0xAF))
	{
		state=2;
		RxBuffer[1]=data;
	}
...
void DT_Data_Receive_Anl(uint8 *data_buf,uint8 num)
{
	...
	if(!(*(data_buf)==0xAA && (*(data_buf+1)==0xAA || *(data_buf+1)==0xAF)))		return;		//ÅÐ¶ÏÖ¡Í·
...
//        g_Car.set_car_speed = (uint16)(*(data_buf+5)<<8)|*(data_buf+6);
    ...
    if(*(data_buf+2)==11)				
    {
       g_Car.pull_max = (uint16)(*(data_buf+4)<<8)|*(data_buf+5);
       g_Car.pull_min = (uint16)(*(data_buf+6)<<8)|*(data_buf+7);

    }
    if(*(data_buf+2)==12)							
    {	 
	   g_Car.control_dir = *(data_buf+4);
       if(g_Car.control_dir == 4)
       {
         g_Car.control_flag = 0;
       }
       else
       g_Car.control_flag = 1;
    }
	if(*(data_buf+2)==13)							
	{
        g_Car.control_speed = (uint16)(*(data_buf+4)<<8)|*(data_buf+5);
        PWM_1_WritePeriod(g_Car.control_speed);
	}
	if(*(data_buf+2)==14)							
	{
	//	DT_Send_Check(*(data_buf+2),sum);
